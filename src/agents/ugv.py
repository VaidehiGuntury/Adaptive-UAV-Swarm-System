"""
Ground vehicle (UGV) agent stub for Paper 2 group leaders.

Implements kinematics only; consensus formation control is deferred.
"""
...
class UGV(BaseAgent):
    """
    UGV group leader on the 2D horizontal plane (Paper 2).

    Tracks a navigation target with bounded speed — placeholder until
    Eq. (6) formation controller is implemented.
    """
    ...
    def role(self) -> AgentRole:
        return AgentRole.UGV

    def set_target(self, target: NDArray[np.float64]) -> None:
        """Assign navigation target for stub motion."""
