...
def _load_search_config(raw_search: dict[str, Any] | None) -> SearchConfig | None:
    ...
    return SearchConfig(
        ...
        targets=TargetSpawnConfig(
            ...
            count_random_walk=int(ts.get("count_random_walk", 0)),
            count_waypoint_patrol=int(ts.get("count_waypoint_patrol", 0)),
            ...
            random_walk_turn_rad=float(ts.get("random_walk_turn_rad", 0.5)),
            waypoint_count=int(ts.get("waypoint_count", 4)),
        ...
        handover=HandoverConfig(
            handover_enabled=bool(ho.get("handover_enabled", True)),
            handover_distance_ratio=float(ho.get("handover_distance_ratio", 0.6)),
            handover_hysteresis_s=float(ho.get("handover_hysteresis_s", 10.0)),
        ),
    )


def _load_handover_config(raw: dict[str, Any] | None) -> "HandoverConfig":
    from src.config.search_config import HandoverConfig
    if raw is None:
        return HandoverConfig()
    return HandoverConfig(
        handover_enabled=bool(raw.get("handover_enabled", True)),
        handover_distance_ratio=float(raw.get("handover_distance_ratio", 0.6)),
        handover_hysteresis_s=float(raw.get("handover_hysteresis_s", 10.0)),
    )
