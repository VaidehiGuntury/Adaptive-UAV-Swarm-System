    ...
    def update(
        ...
        if tid is not None:
            ...
            if event == "completed":
                ...
                if state._direct_nav is not None:
                    state._direct_nav.reset()
            elif event == "handover":
                # This agent may have just received a new target from handover
                # Check if the target is now assigned to this agent
                if target.assigned_uav == agent.agent_id:
                    state.phase = AgentSearchPhase.TRACKING
                    state.current_behaviour = SearchBehaviour.DIRECT_NAV
                    state._direct_nav = DirectNavBehaviour(target)
